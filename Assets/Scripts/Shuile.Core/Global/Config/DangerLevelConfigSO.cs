namespace Shuile.Core.Global.Config
{
    ...
    public static class DangerLevelConfigClass
    {
        ...
        {
            new() { min = 0f, max = 50f }, new() { min = 50f, max = 150f }, new() { min = 150f, max = 500f },
            new() { min = 500f, max = 1000f }, new() { min = 1000f, max = 3000f }
        };
    ...
    public static class DangerLevelUtils
    {
        ...
        {
            var levelRanges = DangerLevelConfigClass.levelRanges;
            if (levelRanges.Length <= 0)
            {
                return -1;
            }

            if (score < levelRanges[0].min)
            {
                return 0;
            }

            if (score > levelRanges[^1].max)
            {
                return levelRanges.Length - 1;
            }
