...
namespace Shuile.Chart
{
    ...
    {
        // note to hit
        private readonly ObjectPool<SingleNote> _notePool = new ObjectPool<SingleNote>(
            ()=>new SingleNote(0f));
        ...
        public void CheckRelease(float currentTime)
        {
            ...
            // 检查所有需要销毁的note
            while (_noteList.Count > 0 && _noteList[0].NeedRelease(currentTime, ImmutableConfiguration.Instance.MissToleranceInSeconds))
            {
