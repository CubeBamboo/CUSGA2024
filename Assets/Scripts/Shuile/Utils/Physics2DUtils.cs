...
namespace Shuile
{
    ...
    {
        public static readonly Vector2 zeroAngleVector = new Vector2(Mathf.Cos(0), Mathf.Sin(1));
        ...
        public static Collider2D[] OverlapFan(Vector2 origin, float from, float to, float radius, float arcStep = 0.1f)
        {
            ...
            {
                foreach (var collider in Physics2D.RaycastAll(origin, new Vector2(Mathf.Cos(angle), Mathf.Sin(angle)), radius).Select(hit => hit.collider))
                    set.Add(collider);
            }
        }

        public static Collider2D[] OverlapFan(Vector2 origin, Vector2 from, Vector2 to, float radius, float arcStep = 0.1f)
            => OverlapFan(origin, Vector2.Angle(zeroAngleVector, from), Vector2.Angle(zeroAngleVector, to), radius, arcStep);
    }
}