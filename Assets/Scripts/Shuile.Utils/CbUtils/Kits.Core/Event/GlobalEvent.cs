...
namespace Shuile.Framework
{
    ...
    [Obsolete("use ModuleContainer.TypeEventSystem")]
    ...
    public class GlobalEvent<TTarget, T1> where TTarget : GlobalEvent<TTarget, T1>
    {
        ...
        public static void Register(Action<T1> action)
        {
            _onAction += action;
        }

        public static void UnRegister(Action<T1> action)
        {
            _onAction -= action;
        }

        public static void Trigger(T1 para1)
        {
            _onAction?.Invoke(para1);
        }
    ...
    public class GlobalEvent<TTarget, T1, T2> where TTarget : GlobalEvent<TTarget, T1, T2>
    {
        ...
        public static void Register(Action<T1, T2> action)
        {
            _onAction += action;
        }

        public static void UnRegister(Action<T1, T2> action)
        {
            _onAction -= action;
        }

        public static void Trigger(T1 para1, T2 para2)
        {
            _onAction?.Invoke(para1, para2);
        }
